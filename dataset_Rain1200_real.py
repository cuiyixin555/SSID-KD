...
class TrainValDataset(Dataset):
    def __init__(self, name):
        ...
        self.root_dir = os.path.join(settings_Rain1200_real.data_dir, name)
        self.real_dir = os.path.join(settings_Rain1200_real.data_dir, 'Real')
        self.mat_files = os.listdir(self.root_dir)
        self.mat_real = os.listdir(self.real_dir)
        self.patch_size = settings_Rain1200_real.patch_size
    ...
    def __getitem__(self, idx):
        ...
        if settings_Rain1200_real.aug_data:
...
class ShowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if settings_Rain1200_real.pic_is_pair:
            O = np.transpose(img_pair[:, w:], (2, 0, 1))
            B = np.transpose(img_pair[:, :w], (2, 0, 1))
        else:
