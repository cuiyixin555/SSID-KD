...
import settings_Rain1200_real as settings
from dataset_Rain1200_real import TrainValDataset, TestDataset
...
class Session:
    def __init__(self):
        ...
        dict_net = torch.load('/media/ubuntu/Seagate/ACM_MM/SemiDerain-master/trained_model/Rain1200/net_latest')
    ...
    def inf_batch(self, name, batch):
        ...
        if self.ssim_loss == True:
            ...
            # percepLoss = 10 * self.perceptualLoss.get_loss(syn_derain, B)
            ...
            # logp_syn = F.log_softmax(torch.add(syn_rain, B), dim=1)
            # p_real = F.softmax(torch.add(real_rain, B), dim=1)

            logp_syn_code = F.log_softmax(syn_code)
            p_real_code = F.softmax(real_code)
            # kl_streak_loss = 1e-5 * F.kl_div(logp_syn, p_real, reduction='mean') + 1e-5 * F.kl_div(logp_syn_code, p_real_code, reduction='mean')
            kl_streak_loss = 1e-6 * F.kl_div(logp_syn_code, p_real_code, reduction='mean')
            # loss = recon_loss + percepLoss + dcLoss + tvLoss + kl_streak_loss
...
def run_train_val(ckp_name_net='net_latest'):
    ...
    while sess.step < settings.total_step + 1:
        ...
        if sess.step % (settings.one_epoch) == 0:
            ...
            logfile = open('../log_test/' + 'Rain1200_real_val' + '.txt', 'a+')
