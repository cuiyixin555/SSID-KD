...
import settings_Rain200L as settings
from dataset_Rain200L import TrainValDataset, TestDataset
...
class Session:
    def __init__(self):
        ...
        self.sche_net = MultiStepLR(self.opt_net, milestones=[settings.l1, settings.l2], gamma=0.5)
...
def run_train_val(ckp_name_net='net_latest'): # 'net_latest'
    ...
    while sess.step < settings.total_step + 1:
        ...
        if sess.step % (settings.one_epoch) == 0:
            ...
            logfile = open('../log_test/' + 'val_Rain200L' + '.txt', 'a+')
