...
class TrainValDataset(Dataset):
    def __init__(self, name):
        ...
        self.root_dir = os.path.join(settings_Rain200H.data_dir, name)
        self.mat_files = os.listdir(self.root_dir)
        self.patch_size = settings_Rain200H.patch_size
...
class ShowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if settings_Rain200H.pic_is_pair:
            O = np.transpose(img_pair[:, w:], (2, 0, 1))
            B = np.transpose(img_pair[:, :w], (2, 0, 1))
        else:
